...
botao_gerar = st.button("🚀 GERAR PLANILHA")

# --- FUNÇÕES DE LÓGICA ---

def remover_acentos(texto):
    """Remove acentos para facilitar a detecção de comércio"""
...
def higienizar_nomes_fortaleza(endereco):
    """Corrige grafias comuns (S por Z) para ajudar o GPS do Circuit"""
    e = str(endereco).upper()
    correcoes = {
        "QUEIROS": "QUEIROZ",
        "LUIS MENDES": "LUIZ MENDES",
        "LUIS": "LUIZ",
        "II": "2",
        "JULIO J ": "JULIO JORGE ",
    ...
    for erro, certo in correcoes.items():
        e = e.replace(erro, certo)
    return e

def identificar_comercio(endereco):
    """Detector de comércio com lógica de contexto"""
    termos_comerciais = [
...
if arquivo_upload is not None and gaiola_alvo and (botao_gerar or st.session_state.dados_rota is not None):
    if botao_gerar:
        with st.spinner('🔄 Filtrando e higienizando...'):
            try:
                ...
                for aba in xl.sheet_names:
                    ...
                    if col_gaiola_idx is not None:
                        ...
                        e_v = dados_filtrados[col_end_idx].astype(str).apply(higienizar_nomes_fortaleza)
